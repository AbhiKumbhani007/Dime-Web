...
import { getMe, updateMe, deleteMe } from '@/lib/server/auth/auth.service'
import { UpdateMeBodySchema } from '@/lib/server/auth/auth.schema'
...
export async function GET(request: Request) {
  try {
    ...
    const user = await getMe(prisma, userId)
    if (!user) {
...
export async function PATCH(request: Request) {
  try {
    ...
    try {
      const user = await updateMe(prisma, userId, parsed.data)
      return NextResponse.json(user, { status: 200 })
    } catch (err) {
