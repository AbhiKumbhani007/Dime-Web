...
import { googleAuth } from '@/lib/server/auth/auth.service'
import { GoogleAuthBodySchema } from '@/lib/server/auth/auth.schema'
...
export async function POST(request: Request) {
  try {
    ...
    try {
      const { tokens, user } = await googleAuth(prisma, parsed.data.idToken)
