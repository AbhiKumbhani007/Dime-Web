...
import { getBudgetVsActual } from '@/lib/server/analytics/analytics.service'
...
export async function GET(request: Request) {
  try {
    ...
    // No query params — reuses budgets' listBudgets (Feature 3) for the current period's computation.
    const budgets = await getBudgetVsActual(prisma, userId)
