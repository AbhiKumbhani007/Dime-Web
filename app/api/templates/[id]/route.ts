...
export async function PATCH(request: Request, context: RouteContext) {
  try {
    ...
    const paramsParsed = TemplateParamsSchema.safeParse(params)
    ...
    const bodyParsed = UpdateTemplateBodySchema.safeParse(body)
    ...
    try {
      ...
      if (e.statusCode === 404) {
        return apiError(404, 'NOT_FOUND', e.message ?? 'Template not found')
      }
...
export async function DELETE(request: Request, context: RouteContext) {
  try {
    ...
    try {
      await deleteTemplate(prisma, userId, paramsParsed.data.id)
