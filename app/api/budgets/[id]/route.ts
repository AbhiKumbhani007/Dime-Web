...
import {
  updateBudget,
  deleteBudget,
} from '@/lib/server/budgets/budgets.service'
import {
  UpdateBudgetBodySchema,
  BudgetParamsSchema,
} from '@/lib/server/budgets/budgets.schema'
...
export async function PATCH(request: Request, context: RouteContext) {
  try {
    ...
    const bodyParsed = UpdateBudgetBodySchema.safeParse(body)
...
export async function DELETE(request: Request, context: RouteContext) {
  try {
    ...
    try {
      await deleteBudget(prisma, userId, paramsParsed.data.id)
