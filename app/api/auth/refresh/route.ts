...
import { refreshTokens } from '@/lib/server/auth/auth.service'
import { RefreshBodySchema } from '@/lib/server/auth/auth.schema'
...
export async function POST(request: Request) {
  try {
    ...
    try {
      const tokens = await refreshTokens(prisma, parsed.data.refreshToken)
