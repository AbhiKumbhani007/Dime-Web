...
import { listPeople, createPerson } from '@/lib/server/ledger/ledger.service'
import { CreatePersonBodySchema } from '@/lib/server/ledger/ledger.schema'
...
export async function GET(request: Request) {
  try {
    ...
    const result = await listPeople(prisma, userId)
...
export async function POST(request: Request) {
  try {
    ...
    const parsed = CreatePersonBodySchema.safeParse(body)
    if (!parsed.success) {
      return apiError(
        400,
        'VALIDATION_ERROR',
    ...
    try {
      const person = await createPerson(prisma, userId, parsed.data)
      ...
      if (e.statusCode === 409) {
        return apiError(
          409,
          'CONFLICT',
          e.message ?? 'A person with this name already exists'
        )
      }
      return apiError(500, 'INTERNAL_ERROR', 'An unexpected error occurred')
    }
