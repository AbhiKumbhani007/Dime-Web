...
import { getTopDays } from '@/lib/server/analytics/analytics.service'
import { TopDaysQuerySchema } from '@/lib/server/analytics/analytics.schema'
...
export async function GET(request: Request) {
  try {
    ...
    const parsed = TopDaysQuerySchema.safeParse(query)
    if (!parsed.success) {
      return apiError(
        400,
        'VALIDATION_ERROR',
    ...
    const days = await getTopDays(prisma, userId, parsed.data)
