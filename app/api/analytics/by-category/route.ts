...
import { getByCategory } from '@/lib/server/analytics/analytics.service'
import { ByCategoryQuerySchema } from '@/lib/server/analytics/analytics.schema'
...
export async function GET(request: Request) {
  try {
    ...
    const parsed = ByCategoryQuerySchema.safeParse(query)
    if (!parsed.success) {
      return apiError(
        400,
        'VALIDATION_ERROR',
    ...
    const categories = await getByCategory(prisma, userId, parsed.data)
